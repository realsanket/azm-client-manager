...
function Invoke-Run {
    ...
    # ── Read-only enforcement: block destructive Azure CLI operations ──
    $blockedActions = @("create", "delete", "update", "set", "start", "stop", "restart", "redeploy", "deallocate", "remove", "add", "deploy", "assign", "revoke", "regenerate", "reset", "move", "swap", "resize", "scale", "import", "attach", "detach", "enable", "disable", "cancel", "invoke", "apply", "restore", "failover", "migrate", "purge", "write", "put", "patch")
    # Extract positional args (everything before first flag) to avoid false positives
    $positionalArgs = @()
    foreach ($a in $cmdArgs) {
        if ($a.StartsWith("-")) { break }
        $positionalArgs += $a
    }
    foreach ($pArg in $positionalArgs) {
        if ($blockedActions -contains $pArg.ToLower()) {
            Exit-WithError "Read-only mode: '$pArg' operations are not permitted.`nOnly read-only commands (list, show, get, export, etc.) are allowed."
        }
    }
...
function Show-Help {
  ...
  ╔═══════════════════════════════════════════════════════════╗
  ║   azm — Azure Multi-Client CLI Manager (PS, AZ-SAFE)    ║
  ╚═══════════════════════════════════════════════════════════╝
  ...
  COMMANDS:
    ...
    run <name> <command...>
        ...
        ⚠️  Only read-only Azure CLI commands are permitted (list, show, get, export).
    ...
    compare <name1> <name2> <command...>
        ...
        ⚠️  Only read-only Azure CLI commands are permitted.
  ...
  EXAMPLES:
    ...
    # Run read-only Azure CLI commands only
    azm run mycompany az group list -o table
    azm compare mycompany othercompany az group list -o json

  AZURE CLI SAFETY:
    This tool blocks destructive Azure CLI commands (create, delete, update, deploy, etc.)
    to prevent accidental resource modifications. Only read-only operations are permitted.
